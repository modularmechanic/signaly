...
pseudo-element pointer targets, and the note above them has always said exactly that — *a 44px touch
target without scaling the socket art*. The coarse block was contradicting its own design note.
...
in visible affordance and was chosen knowingly. The lever for wanting bigger hardware on touch is now
`--hp` at rack level (ADR-0005), which moves the art and the layout together instead of only one of
them.
