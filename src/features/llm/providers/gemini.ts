...
// The key rides in the query string here — that is Gemini's documented browser shape.
const endpoint = (model: string, method: string, key: string): string =>
  `${BASE}/models/${encodeURIComponent(model.replace(/^models\//, ''))}:${method}?key=${encodeURIComponent(key)}`;
...
export const provider: Provider = {
  ...
  async listModels(key) {
    const data = await getJson(
      `${BASE}/models?pageSize=200&key=${encodeURIComponent(key)}`,
      {},
      'Gemini models',
    );
