...
const seg = (a: Pt, b: Pt, kind: Kind, extra?: Partial<Seg>): Seg => {
  ...
  return {
    ...
    c: {
      x: (a.x + b.x) / 2,
      y: (a.y + b.y) / 2 + SAG + sag * SAG_JITTER + Math.hypot(b.x - a.x, b.y - a.y) * 0.16,
    },
    kind,
...
export function overlayView(i: OverlayInput): OverlayView {
  ...
  for (const c of i.cables) {
    ...
    if (!out || !inp) continue;
    segs.push(seg(out.at, inp.at, out.kind, { id: c.id }));
  ...
  if (i.drag) {
    ...
    if (at)
      segs.push(seg(fromFixed ? at.at : mouse, fromFixed ? mouse : at.at, fixed.def.kind, { alpha: 0.85 }));
    sig += `|d${x | 0},${y | 0},${fixed.def.kind}`;
  }
  // A drag owns the pointer: nothing is hoverable until it ends.
  const hover = i.drag ? null : hitTest(segs, i);
