...
describe('forced colours', () => {
  const blocks = atRules(base).filter((r) => r.prelude.includes('forced-colors'));
  const fc = blocks[0];

  it('lives in exactly one block, so no rule escapes these guards in a second one', () => {
  ...
  it('restores a focus indicator the stripped box-shadow can no longer draw', () => {
    ...
    // panel.css swaps the faceplate's outline for an inset box-shadow at (0,2,0), which beats the
    // bare :focus-visible above; the override has to out-specify it or the panel loses its ring.
    expect(fc?.body).toMatch(/:root \.module-panel:focus-visible \{ outline: 2px solid Highlight;/);
  });
