...
const NAMES = ['MAJ', 'MIN', 'MAJ7', 'MIN7', 'DOM7', 'SUS4', 'DIM7', 'AUG', '5TH', 'MAJ SCL', 'MIN SCL', 'PENTA'];
...
class Arp extends Base {
  ...
  advance(
    ...
    const chI = clamp(Math.round((p.chord ?? 0) + (ccv?.[i] ?? 0) * (CHORDS.length / 10)), 0, CHORDS.length - 1);
