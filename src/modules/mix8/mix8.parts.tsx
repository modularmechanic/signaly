...
function curvePath(m: ModuleInstance): string {
  ...
  for (let b = 0; b < NB; b++) {
    biquad(b, m.vals[`eq${b + 1}f`] ?? 1000, m.vals[`eq${b + 1}g`] ?? 0, m.vals[`eq${b + 1}q`] ?? 0.9, sr, co, b * 5);
  }
  return FREQ.map((f, i) => {
    let db = 0;
  ...
  return (
    <div className="mix8-eq">
      ...
      <div className="scope-screen mix8-curve">
        <svg viewBox="0 0 100 60" preserveAspectRatio="none" role="img" aria-label={`EQ curve, channel ${sel + 1}`}>
