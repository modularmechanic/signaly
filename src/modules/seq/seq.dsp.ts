...
/** SEQ-8 — one 8-step pattern of pitch (semitones) + gate.
    In: `{t:'step', i, pitch, gate}` (single edit) or `{t:'steps', v[], g[]}` (patch load).
    Out: `{t:'step', i}` whenever the playhead moves. */
class Seq extends Base {
  vals = [0, 3, 7, 10, 12, 7, 3, -2];
  gates = [1, 1, 1, 1, 1, 0, 1, 1];
  step = 0;
  lc = 0;
  lr = 0;
  cv = 0;
  ...
  override msg(m: InMsg): void {
    if (m.t === 'step') {
      ...
      // A short message updates only the prefix it carries, so a partial edit
      // cannot leave later steps undefined.
  ...
  process(I: Float32Array[][], O: Float32Array[][]): boolean {
    ...
    for (let i = 0; i < cv.length; i++) {
      ...
      this.lr = r;
      if (c > 2.5 && this.lc <= 2.5) this.step = (this.step + 1) % len;
      this.lc = c;
